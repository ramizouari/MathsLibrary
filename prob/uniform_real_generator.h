...
namespace math_rz
{
	...
	public:
		...
		template<int n>
		coordinate_space<real_field, n> generate_vector()
		{
			coordinate_space<real_field, n> X;
			for (int i = 0; i < n; i++)
				X[i] = generate();
			return X;
		}
		...
		matrix<real_field, n, m> generate_matrix()
		{
			matrix<real_field, n, m> X;
			for (int i = 0; i < n; i++)
				for (int j = 0; j < m; j++)
					X[i][j] = generate();
			return X;
		...
		template<int n>
		square_matrix<real_field, n> generate_matrix()
		{
